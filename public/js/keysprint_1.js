...
function resetGame() {
    ...
    clearInterval(timer); // Stop the ongoing timer
    timeLeft = maxTime; // Reset the timer to its maximum value
    charIndex = mistakes = isTyping = 0; // Reset all variables
    inpField.value = ""; // Clear the input field
    timeTag.innerText = timeLeft; // Update the displayed time
    wpmTag.innerText = 0; // Reset the WPM display
    mistakeTag.innerText = 0; // Reset the mistake count
    cpmTag.innerText = 0; // Reset the CPM display
    inpField.disabled = false; // Re-enable the input field
}


loadParagraph(); // Load a random paragraph on page load
inpField.addEventListener("input", initTyping); // Listen for user input
